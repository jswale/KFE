...
(function() {
	...
	GM_define = function(arg1, arg2) {
		...
		if (typeof (arg2) === 'function') // call their define() with the modules
			...
			console.log('GM_define: malformed arguments');
		return null;
	};
...
*/
	...
	GM_require.getModuleUrl = function (modulePathName) {
		if (!modulePathName) { // bail if we have no input
	...
	{
		if (inputConf.baseUrl)
			GM_require.configData.baseUrl = inputConf.baseUrl;
		if (inputConf.paths)
	...
	GM_require.moduleSourceCacheGet = function (moduleName)               { return GM_require.moduleSourceCache[moduleName]; };
	GM_require.moduleSourceCachePut = function (moduleName, moduleSource) { return GM_require.moduleSourceCache[moduleName] = moduleSource; };
	GM_require.moduleSourceCacheDelete = function (moduleName)            { return GM_require.moduleSourceCachePut(moduleName, null); };

	// Module cache methods
	GM_require.moduleCache = {};
	GM_require.moduleCacheGet = function (moduleName)         { return GM_require.moduleCache[moduleName]; };
	GM_require.moduleCachePut = function (moduleName, module) { return GM_require.moduleCache[moduleName] = module; };
	GM_require.moduleCacheDelete = function (moduleName)      { return GM_require.moduleCachePut(moduleName, null); };
